...
const getDailyTasks = async (date) => {
	...
	const datedTasks = await Task.find({
		owner: "octavia",
		date: { $gte: dayBefore, $lt: dayAfter },
		recurring: false,
	});
...
const getMonthlyTasks = async (date) => {
	...
	const monthAfter = new Date(date.getFullYear(), date.getMonth() + 1, 1);
	const datedTasks = await Task.find({
		owner: "octavia",
		date: { $gte: month, $lt: monthAfter },
		recurring: false,
	});
	return datedTasks;
};
